	...
	client.on('group-participants-update', async (anu) => {
		...
		try {
			...
			if (anu.action == 'add') {
				...
				teks = `VAI PELAS SOMBRAS @${num.split('@')[0]} FUCK`
				let buff = await getBuffer(ppimg)
				client.sendMessage(mdata.id, buff, MessageType.image, {caption: teks, contextInfo: {"mentionedJid": [num]}})
			}
		} catch (e) {
		...
		try {
			...
			mess = {
				...
				only: {
					...
					userB: `──「 LISTA 」──\nOlá Kak !\nDesculpe, irmã. Você não está registrado como amigo de kant. Registre-se para fazer amizade com o bot kant por meio, \n\nCommand : ${prefix}daftar nama|idade\nExemplo : ${prefix}daftar nabuto|14\n\n──「 kant BOT 」──`,
			...
			const mod = [ownerNumber,"5571982507697@s.whatsapp.net"]//ubah nomor lo
			const adminbotnumber = ["5571982507697@s.whatsapp.net"]//ubah nomor lo
			const frendsowner = ["5571982507697@s.whatsapp.net"]//ubah nomor lo
			const premium = ["5571982507697@s.whatsapp.net","557182507697@s.whatsapp.net","557599941062@s.whatsapp.net","+14699015476@s.whatsapp.net","5511949051934@s.whatsapp.net","554792091566@s.whatsapp.net","558699541889@s.whatsapp.net","559294313229@s.whatsapp.net","5571982507697@s.whatsapp.net"]
...
break
				...
				case 'wibu':
                   ...
                   break
					case 'totaluser':
					if (!isOwner) return reply(mess.only.ownerB)    
					teks = `\`\`\`╭────*「 *TOTAL DE USUÁRIOO  KANT BOT 👑* 」\n\`\`\``
					no = 0
...
// recursos anime, random @nabuto domina vida
          ...
          reply('Espere..')
			    ...
			    case 'des':
                    ...
                    client.sendMessage(from, 'Meu dono é esse gasoso aí tal de "kant"🙄, chama ele aí ele diz que é "Shitpost"',MessageType.text, { quoted: mek} )
                    break
                case 'fitnah':	
				case 'fake':          
                    if (!isGroup) return reply(mess.only.group)
				...
				case 'neko':
					...
					client.sendMessage(from, tod, image, { quoted: mek, caption: '*╭────*「 *ADMINBOT KANT ✨* 」\n*│+ wa.me/5571982507697*╰──────*「 *KANT* 」*────*\n\n*_SE QUER SER ADMIN DO BOT KAT_*\n*_Tipo /iklan_*' })
					break
				case 'iklan':
